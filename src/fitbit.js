...
const token = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyMkJNRjIiLCJzdWIiOiI4OVA4TDgiLCJpc3MiOiJGaXRiaXQiLCJ0eXAiOiJhY2Nlc3NfdG9rZW4iLCJzY29wZXMiOiJyaHIgcnNldCIsImV4cCI6MTU5MzQ3MzczMSwiaWF0IjoxNTkwODgxNzMxfQ.AW776Z2qRquBqeO-KiECQGxXl2PJm5Eml9N6Cwc-EhI"

const site = "https://api.fitbit.com"
const heartEndpoint = '/1/user/-/activities/heart'

export function heartUrl(start, end) {
  return (site + heartEndpoint +
...
function jsonToTimeSeries(start){
  return function(json) {
    ...
    for(var i = 0; i < hrData.length;  i++){
      ...
      if(prevMo && thisMo < prevMo){ // day has changed over
        date.add(1, 'd');
        thisMo.add(1, 'd');
      }
...
export function getHeartData(start, end) {
  ...
  return fetch(
    heartUrl(moment(startMo), moment(endMo)),
    { headers: new Headers({'Authorization': 'Bearer ' + token}),
      mode: 'cors',
      method: 'GET'}
  ).then(res => res.json()).then(jsonToTimeSeries(startMo));
}
